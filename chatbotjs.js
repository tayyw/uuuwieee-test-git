...
const le = st('<div class="flex justify-end mb-2 items-end guest-container"><span class="px-4 py-2 mr-2 whitespace-pre-wrap max-w-full chatbot-guest-bubble" data-testid="guest-bubble">'),
	...
	me = (nt(["click"]), st('<span>WPWorkPortal<span>')),
	ve = "#303235",
	we = t => {
		let e, r;
		const s = r => {
	...
	Wr = t => {
		...
		const [n, o] = x(""), [i, a] = x(!1), [l, c] = x(!1), [h, u] = x({}), [p, d] = x([{
			...
			if (o(e), "" !== e.trim()) {
				...
				t.chatflowConfig && (s.tweaks = t.chatflowConfig), b() && (s.socketIOClientId = f());
				const {
					data: i,
					error: l
				} = await $t({
					...
					apiHost: t.apiHost,
					chatflowConfig: t.chatflowConfig,
					body: {"inputs": {"input": s['input']},"tweaks":s['tweaks']}
				});
